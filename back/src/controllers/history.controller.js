...
// VER TODAS LAS CONSULTAS Y LA RESPECTIVA INFORMACION
...
/**
 * @desc Obtiene todas las consultas registradas para un paciente específico,
 * incluyendo sus datos relacionados de Antecedentes, Cuestionario y ExamenFisico.
 * @route GET /api/pacientes/:id_paciente/consultas
 * @access Private (ej. requiere autenticación)
 */
export const getConsultationsByPatientId = async (req, res, next) => {
    // 1. Obtener el id_paciente de los parámetros de la URL
    const { id_paciente } = req.params;

    // 2. Validar que el id_paciente sea proporcionado y sea un número válido
    if (!id_paciente || isNaN(parseInt(id_paciente))) {
        return res.status(400).json({ message: 'Se requiere un ID de paciente válido.' });
    }
    ...
    try {
        ...
        if (result.rows.length === 0) {
            return res.status(404).json({ message: `No se encontraron consultas para el paciente con ID ${id_paciente}.` });
        }

        return res.status(200).json(result.rows);

    } catch (error) {
        console.error(`Error al obtener las consultas para el paciente ID ${id_paciente}:`, error);
        return res.status(500).json({
            message: 'Error interno del servidor al obtener las consultas del paciente.',
            error: error.message
        });
    }
};